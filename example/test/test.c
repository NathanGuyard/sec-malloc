...
Test(my_malloc, low_memory_allocation) {
    size_t size = (size_t)-1; // Taille maximale pour provoquer une allocation échouée
...
Test(my_malloc, aligned_memory_allocation) {
    for (size_t align = 8; align <= 64; align *= 2) {
        char *ptr = my_malloc(align);
        cr_assert_not_null(ptr, "Allocation should succeed for alignment %zu", align);
        cr_assert((size_t)ptr % align == 0, "Memory should be aligned to %zu bytes", align);
        my_free(ptr);
    }
}
...
// Test pour vérifier l'allocation et la libération de mémoire avec des tailles non alignées
Test(my_malloc, non_aligned_sizes) {
    size_t sizes[] = {3, 5, 7, 9, 11, 13, 15, 17, 19};
    for (int i = 0; i < 9; i++) {
        char *ptr = my_malloc(sizes[i]);
        cr_assert_not_null(ptr, "Allocation of %zu bytes should succeed", sizes[i]);
        my_free(ptr);
    }
}
...
// Test pour vérifier l'allocation et la libération de mémoire avec memset de valeurs croissantes
Test(my_malloc, incrementing_memset) {
